...
const t = /\/\/ 2\. FALLBACK PATH: yt-dlp extract via Proxy \/ Render\s+if \(\!forceEngine \|\| forceEngine === "layer4" \|\| forceEngine === "layer3"\) \{/;

const p = `// 1.5. LAYER 2 OVERRIDE (Cloudflare Edge Swarm - Meta only)
...
const updated = content.replace(t, p);
